...
```{r}
# 1. Predict probabilities on your full dataset
#    (or just on your train + test split, whichever you’ll compare)
prob_neg <- predict(mod, newdata = dat_feat, type = "response")
#    prob_neg[i] = P(overall<4 | TF‑IDF of review i)

data$ID <- seq_len(nrow(data))

# 2. Attach back to your original `data` frame by ID
data_with_prob <- data %>%
  mutate(prob_neg = prob_neg[match(ID, dat_feat$ID)]) 

# 3. Now rebuild your recommender features:
...
# 4. Recompute cosine similarities and recommendations as before
...
Plot 1 (baseline) shows almost all similarities clustered in the negative/near‑zero region, with only a tiny spike up near +1.0.

Plot 2 (with our prob_neg feature) has a visibly fatter right‑hand tail (more items in the +0.2 to +1.0 range), meaning more candidates are judged “similar.”
...
# EVERYTHING BELOW THIS WONT BE ON THE FINAL HTML. THOSE ARE STILL UNDER TESTING

------------------------------------------------------------------------

## Deep Learning

For classical text‐based models (like Logistic Regression), we wanted to strip out punctuation, drop stop words (except negations), lowercase everything, etc., because we were turning text into “bags of words” or TF‐IDF vectors.

With a transformer‐based model like DistilBERT, however, we do not want to throw away punctuation, remove stop words, or pre‐tokenize yourself. Instead we feed it the raw or lightly cleaned text and let its own subword tokenizer:

Preserve punctuation and special tokens that carry meaning (e.g. negation, contractions, emojis).

Learn word pieces (e.g. “walking” → “walk” + “##ing”) so we don’t have to manually handle every morphological variant.
...
train_df$cleaned_reviews <- pbapply::pbsapply(train_df$reviewText, clean_text)
val_df$cleaned_reviews   <- pbapply::pbsapply(val_df$reviewText, clean_text)
test_df$cleaned_reviews  <- pbapply::pbsapply(test_df$reviewText, clean_text)


# --- Save results to CSV for reuse ---
fwrite(train_df, "dl_train_cleaned.csv")
fwrite(val_df,   "dl_val_cleaned.csv")
fwrite(test_df,  "dl_test_cleaned.csv")
...
# Tell reticulate where to find conda and which env to activate
use_condaenv(
  conda     = "C:/Users/User/miniconda3/Scripts/conda.exe",
  condaenv  = "r-tf",
  required  = TRUE
)
...
# reticulate::py_install(c("transformers","tensorflow"), envname = "r-reticulate")
...
# --- Load Cleaned Data ---
train_df <- fread("dl_train_cleaned.csv")
val_df   <- fread("dl_val_cleaned.csv")
test_df  <- fread("dl_test_cleaned.csv")
...
# 4) Tokenize all splits (pad/truncate to max_length=200)
tokenize_batch <- function(texts) {
  tokenizer(
    texts,
    truncation     = TRUE,
    padding        = "max_length",
    max_length     = as.integer(200L),
    return_tensors = "tf"
  )
}

train_enc <- tokenize_batch(train_df$cleaned_reviews)
val_enc   <- tokenize_batch(val_df$cleaned_reviews)
test_enc  <- tokenize_batch(test_df$cleaned_reviews)

# Prepare x & y for train/val
train_x <- list(
...
# 7) Prepare integer label tensors (ensure 0-based)
#    If your R sentiment is 1,2,3 you must subtract 1 to get 0,1,2
train_y <- tf$constant(as.integer(train_df$sentiment) - 1L, dtype = tf$int32)
val_y   <- tf$constant(as.integer(val_df$sentiment)   - 1L, dtype = tf$int32)
test_y  <- tf$constant(as.integer(test_df$sentiment)  - 1L, dtype = tf$int32)
...
model     <- transformers$TFDistilBertForSequenceClassification$from_pretrained(
  "distilbert-base-uncased", num_labels = as.integer(3)
)

# 1) Now that tf_keras is installed, this compile should succeed
model$compile(
...
# 3) Fit with a progress bar (verbose = 1) and CSV logging
history <- model$fit(
  x               = train_x,
  y               = train_y,
  ...
  epochs          = as.integer(3),
  verbose         = as.integer(1),       # shows the tqdm-like progress
  callbacks       = list(csv_logger)
)

# 4) Save training log for inspection later
training_log <- read_csv("training_log.csv")
saveRDS(training_log, "training_log.rds")

# 5) Evaluate on test set (progress bar too)
results <- model$evaluate(
  x       = test_x,
  y       = test_y,
  verbose = as.integer(1)
)
names(results) <- c("loss", "accuracy")

# 6) Persist final test metrics
saveRDS(results, "test_results.rds")

# 7) (Optionally) save the model & tokenizer
...
1.  Use classification probabilities (from your logistic model) as a proxy for implicit user ratings.
2.  Convert these scores into a realRatingMatrix.
3.  Use collaborative filtering (IBCF or UBCF) on the matrix built from predicted probabilities.
4.  Use evaluationScheme() for model validation
...
df <- df %>%
  mutate(
    sentiment_num = case_when(sentiment == "positive" ~ 1,
                              sentiment == "neutral" ~ 0,
                              sentiment == "negative" ~ -1),
    length_num = case_when(length == "small" ~ 1,
                           length == "medium" ~ 2,
                           length == "large" ~ 3),
    target = ifelse(overall >= 4, 1, 0)
  )
...
| Column          | Purpose                                                                                                                                                              |
|--------------------------|----------------------------------------------|
| `target`        | This is the label — a binary variable indicating whether a review is **"positive"** (≥ 4 stars). This is what we're trying to **predict**.                           |
| `sentiment_num` | This is a **feature** (input). It encodes the `sentiment` label (which was created from the star rating) into numeric form, so it can be used in a model if desired. |
| `length_num`    | Another **feature** (input). It encodes the length of the review (which we calculated earlier in the Classification Model Section) as a numeric variable.            |

In this context, sentiment_num and length_num are just handcrafted features added to help the classification model.

## Classification Model + Predicted Probabilities

```{r}
# --- Fit Classification Model (Structured Features Only) ---
model <- glm(target ~ sentiment_num + length_num + overall,
             data = df, family = quasibinomial())

# --- Predict Preference Score for Each User-Item ---
df$predicted_prob <- predict(model, newdata = df, type = "response")

```

## Matrix Construction (Subsample + Scoring)

```{r}
# --- Create Fake User IDs if Missing ---
if (!"reviewerID" %in% colnames(df)) {
  df$reviewerID <- paste0("user", seq_len(nrow(df)))
}

# --- Sample for Manageability ---
set.seed(42)
df_sub <- df %>%
  filter(reviewerID %in% sample(unique(reviewerID), 300),
         asin %in% sample(unique(asin), 500))

# --- Build User-Item Matrix of Predicted Scores ---
rating_df <- df_sub %>%
  select(reviewerID, asin, predicted_prob) %>%
  group_by(reviewerID, asin) %>%
  summarise(score = mean(predicted_prob), .groups = "drop")

rating_mat <- rating_df %>%
  pivot_wider(names_from = asin, values_from = score, values_fill = 0)

user_ids <- rating_mat$reviewerID
rating_mat$reviewerID <- NULL
rating_matrix <- as(as.matrix(rating_mat), "realRatingMatrix")
rownames(rating_matrix) <- user_ids


```

## Clean & Build Evaluation Scheme

```{r}
# --- Filter Users and Items with Zero Interactions ---
rating_matrix <- rating_matrix[rowCounts(rating_matrix) > 0, ]
rating_matrix <- rating_matrix[, colCounts(rating_matrix) > 0]

# --- Evaluation Setup (as taught in recommender-system.html) ---
set.seed(1031)
es <- evaluationScheme(rating_matrix, method = "split", train = 0.8, given = 10, goodRating = 0.6)
